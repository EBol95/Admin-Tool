...
@WebServlet("/search/studentsbycourse")
public class SearchStudentsByCourseController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			List<Student> students = studentCourseServ.getStudentsByCourse(courseId);
			...
			if (students.size() == 0) {
				...
				request.setAttribute("courseId", courseId);
				request.getRequestDispatcher("/jsps/removestudentfromcourse.jsp")
					.forward(request, response);
			}
