...
@WebServlet("/search/coursesnotinstudent")
public class SearchCoursesNotInStudentController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			List<Course> courses = studentCourseServ.getCoursesNotInStudent(studentId);
			...
			if (courses.size() == 0) {
				...
				request.setAttribute("studentId", studentId);
				request.getRequestDispatcher("/jsps/addcoursetostudent.jsp")
					.forward(request, response);
			}
